        ...
        return array(
            'factories' => array(
                'Alert\Model\AlertTable' =>  function($sm) {
                    $tableGateway = $sm->get('AlertTableGateway');
                    $table = new AlertTable($tableGateway);
                    return $table;
                },
                'AlertTableGateway' => function ($sm) {
                    $dbAdapter = $sm->get('Zend\Db\Adapter\Adapter');
                    $resultSetPrototype = new ResultSet();
                    $resultSetPrototype->setArrayObjectPrototype(new Alert());
                    return new TableGateway('alertmsg', $dbAdapter, null, $resultSetPrototype);
                },
            ),
        );
    }
